...
class ThriftServer extends EventEmitter {
   ...
   *
   *         // use user define thrift handler, disable `services`
   *         [processor]
   *         [handler]
   *         [options]
   *     }
   ...
   * @param {Array|Object} handler array - services, object - service, in each handler:
   *   {
  ...
  add(services) {
    ...
    services.forEach((h) => {
      ...
      let alias   = h.alias,
          service = h.service,
          actions = utils.trans2Array(h.actions, _.isString);
      // alias
      ...
      if (_.isString(alias)) {
        ...
        this._handler[alias] = {origin: service, actions: checks};
        this._addToRedis(alias, {id: this._id, host: this._host, port: this._port, actions: checks});
      }
  ...
  /**
   * init redis and register all services
   * @private
   */
  ...
  _initThriftHandler() {
    ...
    this._innerHandler = {
      call(cmsg, callback) {
        ...
        let base    = cmsg.base,
            caller  = cmsg.call,
            service = that._handler[caller.name];
