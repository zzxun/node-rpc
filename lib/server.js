  ...
  constructor(options) {
    ...
    this._adapter.on(utils.EVENT.READY, () => {
      ...
      utils.getUnusedPort(_.isNumber(options.thrift.port) ? options.thrift.port : PORT)
        ...
        // inital service
        if (this._custom) {
          return this._adapter.publish({alias: options.thrift.alias, id: this._id},
            {host: this._host, port: this._port, actions: false},
            utils.TTL);
        } else {
          this.add = this._add;
          // inital service
        ...
        // emit listening
        this.emit(utils.EVENT.READY, 'ThriftServer host: ' + this._host + ' , port: ' + this._port);
  ...
  _add(services) {
    ...
    return Promises.map(services, (s) => {
      ...
      if (_.isString(alias)) {
        ...
        if (!_.isEmpty(actions)) {
          ...
          actions.forEach((action) => {
            if (service && _.isFunction(service[action])) {
              ...
              this.emit(utils.EVENT.ERROR, 'invalid service or action', 'service: ' + alias + ', action: ' + action);
            }
          });
        }
        // memory
        ...
        // public
        this._adapter.publish({alias: alias, id: this._id},
  ...
  _initThriftHandler() {
    ...
    this._innerHandler = {
      call(cmsg, callback) {
        ...
        self.emit(utils.EVENT.LOG, JSON.stringify({
