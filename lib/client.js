...
class ThriftClient extends EventEmitter {
  ...
  call(alias, action, params) {
    ...
    return this._initBeforeCall(alias)
      .bind(this)
      .then(() => {
        return this._clients[alias] ? this._clients[alias].next() : null;
      })
      .then((client) => {
        if (client) {
          // check permission
      ...
      .map((data) => {
        ...
        try {
          ...
          if (data && data.host && data.port) {
                  ...
                  this._adapter.del({alias: alias, id: data.id})
                    .then(() => {
                      this.emit(utils.EVENT.LOG,
                        alias + '.' + data.id + ': delete client ' + data.host + ':' + data.port);
                    })
                    .catch((err) => {
                      this.emit(utils.EVENT.ERROR,
                        alias + '.' + data.id + ': delete client ' + data.host + ':' + data.port, err);
                    });
                };
            // cannot connect
            ...
            this.emit(utils.EVENT.LOG, alias + '.' + data.id + ': add client ' + data.host + ':' + data.port);
          } else {
