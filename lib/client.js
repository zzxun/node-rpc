      ...
      .map((data) => {
        ...
        try {
          ...
          if (data && data.host && data.port) {
                  ...
                  this._adapter.del({alias: alias, id: data.id})
                    .then(() => {
                      this.emit(utils.EVENT.LOG,
                        alias + ' delete client ' + data.host + ':' + data.port + ' ' + data.id);
                    })
                    .catch((err) => {
                      this.emit(utils.EVENT.ERROR,
                        alias + ' delete client ' + data.host + ':' + data.port + ' ' + data.id, err);
                    });
                };
            // cannot connect
            ...
            this.emit(utils.EVENT.LOG, alias + ' add client ' + data.host + ':' + data.port + ' ' + data.id);
          } else {
