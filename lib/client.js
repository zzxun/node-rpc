...
class ThriftClient extends EventEmitter {
   ...
   */
  _findServices(alias) {
    return this._adapter.getListKeys(utils.KEY({alias: alias, id: '*'}))
      .bind(this)
      .then((keys) => {
        if (_.isEmpty(keys)) {
          return null;
          ...
          if (data && data.host && data.port) {
            ...
            let connection = thrift.createConnection(data.host, data.port),
                ...
                clean      = () => {
                  ...
                  this._adapter.del(utils.KEY({alias: alias, id: data.id}));
            ...
            this.emit(utils.EVENT.ERROR, 'ignore ' + data);
          }
        } catch (e) {
          this.emit(utils.EVENT.ERROR, e);
        }
      });
  }

  /**
   * clean client if server error or server colse
   * @param alias
   * @param data
   * @private
   */
  _cleanClient(alias, data) {
    this._adapter.del(utils.KEY({alias: alias, id: data.id}));
